...
export class CommandHandler {
        ...
        switch (info.name.toLowerCase()) {
            ...
            case "save":
                return "Save your currently equipped items\n" +
                    ...
                    "\t\t : defaults to roubles\n" +
                    "\t\t : amount is a number or dynamic when blank (0 removes price)\n" +
                    "== Examples ==\n" +
            ...
            case "setprice":
                return "Set the price on a loadout\n" +
                    "Usage\t:: setprice -name- (--price -price info-)\n" +
                    "== Parameters ==\n" +
                    "name\t\t:: the name of the loadout\n" +
                    "--price\t:: optionally add a price to the loadout\n" +
                    "price info\t:: optionally specify the money type and amount\n" +
    ...
    private listLoadoutCommand(info: CommandInfo): string {
        const loadoutInfo = this.loadoutManager.getLoadoutListInfo();

        let page: number = 1;

        if (info.name)
        {
            const pageNumber = Number.parseInt(info.name);
        ...
        const pageCount = 5;
        const totalPages = Math.ceil(loadoutInfo.length / pageCount);

        const start: number = ((pageCount) * (page - 1));
        const end: number = start + pageCount;

        const pageData = loadoutInfo.slice(start, end);
        var message: string = `== Saved Loadouts (${page}/${totalPages}) ==\n`;

        for (const info of pageData)
        {
            message += `- ${info}\n`
        }
    ...
    private getLoadoutCommand(info: CommandInfo, sessionId: string, bot: IUserDialogInfo): string {
        ...
        // todo: figure out how tf I'm going to take money from the profile
        //     : chomp says the profile responses are fairly tightly coupled to the client
        //     : see -> PaymentService.payMoney()
        //     : and -> HealthController.healthTreatment() line 182
    ...
    route(sessionId: string, request: ISendMessageRequest, bot: IUserDialogInfo): void {
        ...
        switch (commandInfo.command) {
            ...
            case CommandType.list:
                response = this.listLoadoutCommand(commandInfo);
                break;
